...
const sendMessageController = asyncHandler(async (req, res) => {
    ...
    const isteamMember = team.members.some((member) => member.user.toString() === userId.toString())
...
const getMessageController = asyncHandler(async (req, res) => {
    ...
    const chats = await chatModel
        .find({ team: teamId, deletedAt: null })
        .sort({ createdAt: 1 })
...
const filesController = asyncHandler(async (req, res) => {
    ...
    const chat = await chatModel
        .find({ team: teamId, deletedAt: null, "attachments.0": { $exists: true } })
        .sort({ createdAt: -1 })
        .populate("sender", "name email")
    if (chat.length === 0) {
        return res.status(200).json({
